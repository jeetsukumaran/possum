 ...
 * @param line_width            column to wrap on
 * @first_line_indent           number of spaces to indent first line of text
 * @subsequence_line_indent     number of spaces to indent subsequent line of text
 * @returns                     string containing wrapped text.
 */
...
/**
 * Dummy option (makes OptionArg unabstract by implementing no-op methods. Used
 * mainly to add help entry.)
 */
class DummyOptionArg : public OptionArg {


    public:

        /**
         * Passes everything to base class.
         */
        DummyOptionArg(const char * help=NULL, const char * meta_var=NULL)
         ...
         */
        virtual void process_value_string(const std::string& ) {
        }
...
class OptionParser {
    ...
    public:
        ...
        OptionParser(const char * version=NULL,
                ...
                const char * default_option_group=NULL);
        ...
        OptionArg * add_option(void * store,
            ...
            this->add_option_to_option_group(oa, option_group);
            return oa;
        }

        /**
         * Treats single dash by itself ('-') as an argument (typically used to indicate
         * that data should be read from the standard input).
         */
        void add_single_hyphen_switch(bool * store, const char * help=NULL, const char * option_group=NULL) {
            if (help == NULL) {
                help = "read from standard input";
            }
            DummyOptionArg * osi = new DummyOptionArg(help);
            osi->set_short_flag("-");
            osi->set_is_switch(true);
            this->add_option_to_option_group(osi, option_group);
            this->single_hyphen_switch_ = store;
        }
        ...
        OptionArg * add_switch(void * store,
                               ...
                               const char * meta_var=NULL,
                               const char * option_group=NULL) {
            OptionArg * switch_arg = this->add_option<bool>(store, short_flag, long_flag, help, meta_var, option_group);
    ...
    private:
         ...
         */
        void add_option_to_option_group(OptionArg * oa, const char * option_group=NULL);

        /**
         * Process an option and its argument.
         */
        int handle_value_assignment(OptionArg& oa,
                std::string& arg_name,
            ...
            if (not oa.is_switch()) {
                if (arg_value.size() == 0) {
                    if (current_argv_idx == argc-1) {
                        std::cerr << "Expecting value for option '" << arg_name << "'" << std::endl;
                        exit(1);
                    } else {
                        arg_value = argv[current_argv_idx+1];
                        current_argv_idx += 1;
                    }
                }
                try {
    ...
    private:
        ...
        PosArgs                                     pos_args_;
        /** option args help groups */
        std::vector< const char *>                         option_group_names_;
        std::map< const char *, std::vector<OptionArg *> > option_groups_;
        const char *                                       default_option_group_;
        /** map of flag strings to option arguments */
        std::map< std::string, OptionArg * >        key_opt_map_;
        /** if single-hyphen is given by itself, this is set to true */
        bool *                                      single_hyphen_switch_;
        /** if double-hyphen is given by itself, this is set to true */
        bool                                        double_hyphen_switch_;
